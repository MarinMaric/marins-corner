...
export class DeckGuideService {
    getGuides():DeckGuide[]{
        ...
        atem.general = "The heart of the deck is the Dark Magician and Buster Blader duo. They are the main beaters you will want to go to, and the rest of the cards are mostly there to maintain field presence until you get to them. Skilled Dark Magician, Gold Sarchophagus, Dark Magic Curtain, and Emblem of the Dragon Destroyer are used to search for these two, respectively, if the ample amount of draw power doesn’t get to them first (Pot of Greed, Graceful Charity, and Card Destruction).\n \nMagnet Warriors, Knights, and the Gazelle/Berfomet duo are also here. Chimera is a decent Beatstick which can float into a Berfomet but it is generally preferrable to save Polymerization for Dark Paladin. However, sometimes that isn't an option or you may just want to apply additional pressure quickly so Chimera is a solid option. Summoning Valkyrion is not very likely, so do not focus on that unless the circumstances are such that you have a high chance of getting to the third Magnet Warrior. Valkyrion is thus mostly used as a discard fodder as he is a “Garnet” of the deck. However, Sangan can be used to search Alpha and Gamma while revival spells can bring the dead magnets back, making it a little easier. Sangan can also be used to search Kuriboh and Electromagnetic Turtle.\n\nThe deck runs Brain Control in its pre-errata version, meaning it can steal special summoned monsters as well. These monsters can then be tributed, making this a powerful 2-card combo for removing threats.";
        atem.combos=[];
        atem.mathchups = [
            ...
            {title:'Yusei', imageUrl:'https://i.imgur.com/r1ePgDn.png', description:"Yusei's ace cards are both dragons, so Buster Blader is a good counter, given how Shooting Star Dragon can't run it over. However, Yusei has Junk Berserker and Junk Destroyer that can get over Buster Blader. Yusei can also win games without relying on dragons at all since most of his synchros are Warriors. Additionally, even if he does go for Stardust or Shooting Star, Armory Arm can help him run it over.\n\nWhenever you are in the position to actually summon Valkyrion, definitely do so because it's stats are higher than those of any monster Yusei has. Also, unlike Dark Paladin, Valkyrion can be revived so committing to it once it's summoned is a good strategy. Try to save Brain Control for Stardust and Shooting Star to tribute them. Save S/T removal for Scrap-Iron Scarecrow and Power Frame. \n\nSince Stardust has to tribute itself to negate a destruction effect, you can bait it and use Monster Reborn to steal it. Generally, you don't need to limit yourself to stealing Stardust from his GY. Yusei often synchros Stardust by tuning other Synchros like Armory Arm, Turbo Warrior or Nitro Warrior, all of which can be used to run over Stardust. This is actually a better option since these are all better counters to Yusei's gameplan than Stardust since his most powerful aspect is effect negation which if used will cause Stardust to go back to his field. Meanwhile Turbo Warrior halves the ATK of any synchro it attacks while Nitro Warrior can swing with 3800 ATK every turn you activate a spell."},
        ];
